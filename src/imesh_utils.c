...
#include <sys/types.h>
#include <sys/stat.h>
#include "imesh_utils.h"
#include "imesh_const.h"
#include "imesh_shapes.h"
#include "imesh_core.h"
#include "imesh_bench.h"
 ...
 */
#define imesh_ASSERT(a)  (!(a) && (imesh_error((a),"input error",__FILE__,__LINE__),1) );
...
void imesh_cleargpuinfo(GPUInfo **gpuinfo){
    if(*gpuinfo){
	free(*gpuinfo);
	*gpuinfo=NULL;
    }
...
void imesh_clearcfg(Config *cfg){
     if(cfg->medianum)
     	free(cfg->prop);
     if(cfg->detnum)
     	free(cfg->detpos);
...
void imesh_savenii(float *dat, size_t len, char* name, int type32bit, int outputformatid, Config *cfg){
     FILE *fp;
	...
	hdr.vox_offset = (float)0;
     ...
     if (( fp = fopen(fname,"wb")) == NULL)
             imesh_ERROR(-9, "Error opening header file for write");

     if (fwrite(&hdr, MIN_HEADER_SIZE, 1, fp) != 1)
             imesh_ERROR(-9, "Error writing header file");

     if (outputformatid==ofNifti) {
         if (fwrite(&pad, 4, 1, fp) != 1)
             imesh_ERROR(-9, "Error writing header file extension pad");

         if (fwrite(logval, (size_t)(hdr.bitpix>>3), hdr.dim[1]*hdr.dim[2]*hdr.dim[3]*hdr.dim[4], fp) !=
	          hdr.dim[1]*hdr.dim[2]*hdr.dim[3]*hdr.dim[4])
             imesh_ERROR(-9, "Error writing data to file");
	 fclose(fp);
         ...
         fp = fopen(fname,"wb");
         if (fp == NULL)
             imesh_ERROR(-9, "Error opening img file for write");
         if (fwrite(logval, (size_t)(hdr.bitpix>>3), hdr.dim[1]*hdr.dim[2]*hdr.dim[3]*hdr.dim[4], fp) != 
	       hdr.dim[1]*hdr.dim[2]*hdr.dim[3]*hdr.dim[4])
             imesh_ERROR(-9, "Error writing img file");

         fclose(fp);
     }else
         imesh_ERROR(-9, "Output format is not supported");
}
...
void imesh_savebnii(float *vol, int ndim, uint *dims, float *voxelsize, char* name, int isfloat, Config *cfg){
	 ...
	 ubjw_begin_object(root,UBJ_MIXED,0);
         if(imesh_jdataencode(vol,ndim,dims,(isfloat?"single":"uint32"), 4, cfg->zipid, root, 1, cfg))
	      imesh_ERROR(-1,"error when converting to JSON");
         ubjw_end(root);
     ubjw_end(root);
     ...
     fp=fopen(fname,"wb");
     if(fp==NULL)
	  imesh_ERROR(-1,"error opening file to write");
...
void imesh_savejnii(float *vol, int ndim, uint *dims, float *voxelsize, char* name, int isfloat, Config *cfg){
	...
	cJSON_AddStringToObject(hdr, "Description", typestr[(int)cfg->outputtype]);
     ...
     if(imesh_jdataencode(vol,ndim,dims,(isfloat?"single":"uint32"), 4, cfg->zipid, dat, 0, cfg))
         imesh_ERROR(-1,"error when converting to JSON");
...
void imesh_savedata(float *dat, size_t len, Config *cfg){
     FILE *fp;
     ...
     if(cfg->outputformat==ofNifti || cfg->outputformat==ofAnalyze){
         imesh_savenii(dat, len, name, NIFTI_TYPE_FLOAT32, cfg->outputformat, cfg);
         return;
	 ...
	 if(cfg->seed==SEED_FROM_FILE && cfg->replaydet==-1 && (cfg->detnum>1 || cfg->srcnum>1)){
	     ...
	     if(cfg->outputformat==ofJNifti)
                 imesh_savejnii(dat, 5, dims, voxelsize, name, 1, cfg);
	     else
	         imesh_savebnii(dat, 5, dims, voxelsize, name, 1, cfg);
	 }else{
	     ...
	     if(d1)
	         imesh_convertcol2row(&buf, (uint3 *)dims);
	     else
	         imesh_convertcol2row4d(&buf, (uint4 *)dims);
	     if(cfg->outputformat==ofJNifti)
	         imesh_savejnii((float*)buf, 4-d1, dims, voxelsize, name, 1, cfg);
	     else
	         imesh_savebnii((float*)buf, 4-d1, dims, voxelsize, name, 1, cfg);
	     free(buf);
	 }
         return;
     }
     ...
     if(fp==NULL){
	imesh_ERROR(-2,"can not save data to disk");
     }
...
void imesh_savedetphoton(float *ppath, void *seeds, int count, int doappend, Config *cfg){
	FILE *fp;
	char fhistory[MAX_FULL_PATH], filetag;
	if(cfg->outputformat==ofJNifti || cfg->outputformat==ofBJNifti){
	    imesh_savejdet(ppath,seeds,count,doappend,cfg);
	    return;
	}
...
void imesh_savejdet(float *ppath, void *seeds, uint count, int doappend, Config *cfg){
	...
	for(int i=0;i<cfg->medianum;i++){
		...
		if(imesh_jdataencode(buf,2,dims,dtype[id], 4, cfg->zipid, sub, 0, cfg))
		    imesh_ERROR(-1,"error when converting to JSON");
		free(buf);
		col+=dims[1];
	    }
		...
		cJSON_AddItemToObject(dat, dname[id], sub = cJSON_CreateObject());
		if(imesh_jdataencode(val,2,dims,dtype[id], 4, cfg->zipid, sub, 0, cfg))
		    imesh_ERROR(-1,"error when converting to JSON");
		free(val);
		col+=dims[1];
	      }
	...
	if(cfg->issaveseed && seeds!=NULL){
	    ...
	    if(imesh_jdataencode(seeds,2,dims,"uint8", 1, cfg->zipid, sub, 0, cfg))
                imesh_ERROR(-1,"error when converting to JSON");
	}
...
void imesh_printlog(Config *cfg, char *str){
     if(cfg->flog>0){ /*stdout is 1*/
         imesh_FPRINTF(cfg->flog,"%s\n",str);
     }
}
...
void imesh_normalize(float field[], float scale, int fieldlen, int option, int pidx, int srcnum){
     int i;
 ...
 */
 ...
 void imesh_kahanSum(float *sum, float *kahanc, float input){
 ...
 */
 ...
 float imesh_updatemua(unsigned int mediaid, Config *cfg){
     float mua;
     if(cfg->mediabyte<=4)
...
void imesh_error(const int id,const char *msg,const char *file,const int linenum){
#ifdef imesh_CONTAINER
     imesh_throw_exception(id,msg,file,linenum);
#else
     imesh_FPRINTF(stdout,S_RED "\nMCX ERROR(%d):%s in unit %s:%d\n" S_RESET,id,msg,file,linenum);
     if(id==-CUDA_ERROR_LAUNCH_FAILED){
         imesh_FPRINTF(stdout,S_RED "MCX is terminated by your graphics driver. If you use windows, \n\
...
void imesh_assert(int ret){
     if(!ret) imesh_ERROR(ret,"assert error");
}
...
void imesh_readconfig(char *fname, Config *cfg){
     if(fname[0]==0){
     	imesh_loadconfig(stdin,cfg);
     }else{
        FILE *fp=fopen(fname,"rt");
        if(fp==NULL && fname[0]!='{') imesh_ERROR(-2,"can not load the specified config file");
        if(strstr(fname,".json")!=NULL || fname[0]=='{'){
            ...
            if(fp!=NULL){
                ...
                if(fread(jbuf,len-1,1,fp)!=1)
                    imesh_ERROR(-2,"reading input file is terminated");
                jbuf[len-1]='\0';
            }else
		jbuf=fname;
            jroot = cJSON_Parse(jbuf);
            if(jroot){
                imesh_loadjson(jroot,cfg);
                cJSON_Delete(jroot);
            }else{
                ...
                if(ptr && ptrold){
                   ...
                   while(offs<ptrold){
                      imesh_FPRINTF(stderr,"%c",*offs);
                      offs++;
                   }
                   imesh_FPRINTF(stderr,"<error>%.50s\n",ptrold);
                }
                if(fp!=NULL) free(jbuf);
                imesh_ERROR(-9,"invalid JSON input file");
            }
            if(fp!=NULL) free(jbuf);
        }else{
	    imesh_loadconfig(fp,cfg); 
        }
        if(fp!=NULL) fclose(fp);
	if(cfg->session[0]=='\0'){
	...
	if (stat((const char *)cfg->rootpath, &st) == -1) {
	    if(mkpath(cfg->rootpath, 0755))
	       imesh_ERROR(-9,"can not create output folder");
	}
     }
}
...
void imesh_writeconfig(char *fname, Config *cfg){
     if(fname[0]==0)
     	imesh_saveconfig(stdout,cfg);
     else{
     	FILE *fp=fopen(fname,"wt");
	if(fp==NULL) imesh_ERROR(-2,"can not write to the specified config file");
	imesh_saveconfig(fp,cfg);     
	fclose(fp);
     }
}
...
void imesh_prepdomain(char *filename, Config *cfg){
     if(cfg->isdumpjson==2){
	  imesh_savejdata(cfg->jsonfile, cfg);
	  exit(0);
     }
     if(filename[0] || cfg->vol){
        if(cfg->vol==NULL){
	     imesh_loadvolume(filename,cfg,0);
	     if(cfg->shapedata && strstr(cfg->shapedata,":")!=NULL){
		  ...
		  status=imesh_parse_shapestring(&grid,cfg->shapedata);
		  if(status){
		      imesh_ERROR(status,imesh_last_shapeerror());
		  }
	     }
	}
	if(cfg->isrowmajor){
		/*from here on, the array is always col-major*/
		imesh_convertrow2col(&(cfg->vol), &(cfg->dim));
		cfg->isrowmajor=0;
	}
	...
	if(cfg->isdumpmask)
     	...
     	imesh_ERROR(-4,"one must specify a binary volume file in order to run the simulation");
     }
     if(cfg->respin==0)
        imesh_ERROR(-1,"respin number can not be 0, check your -r/--repeat input or cfg.respin value");
     ...
     if(cfg->seed==SEED_FROM_FILE && cfg->seedfile[0]){
	   ...
	   fprintf(stderr,S_RED "WARNING: respin is disabled in the replay mode\n" S_RESET);
        ...
        imesh_loadseedfile(cfg);
     }
     if(cfg->replaydet>(int)cfg->detnum)
        imesh_ERROR(-4,"replay detector ID exceeds the maximum detector number");
     ...
     for(int i=0;i<6;i++)
        if(cfg->bc[i]>='A' && imesh_lookupindex(cfg->bc+i,boundarycond))
	   imesh_ERROR(-4,"unknown boundary condition specifier");

     if((cfg->mediabyte==MEDIA_AS_F2H || cfg->mediabyte==MEDIA_MUA_FLOAT || cfg->mediabyte==MEDIA_AS_HALF) && cfg->medianum<2)
         imesh_ERROR(-4,"the 'prop' field must contain at least 2 rows for the requested media format");
     if((cfg->mediabyte==MEDIA_ASGN_BYTE || cfg->mediabyte==MEDIA_AS_SHORT) && cfg->medianum<3)
         imesh_ERROR(-4,"the 'prop' field must contain at least 3 rows for the requested media format");
     ...
     if(cfg->issaveref>1){
        if(cfg->issavedet==0)
	    imesh_ERROR(-4,"you must have at least two outputs if issaveref is greater than 1");

        if(cfg->dim.x*cfg->dim.y*cfg->dim.z > cfg->maxdetphoton){
	    imesh_FPRINTF(cfg->flog,"you must set --maxdetphoton larger than the total size of the voxels when --issaveref is greater than 1; autocorrecting ...\n");
...
void imesh_loadconfig(FILE *in, Config *cfg){
     uint i,gates,itmp;
     size_t count;
     float dtmp;
     ...
     imesh_ASSERT(fscanf(in,"%ld", &(count) )==1);
     if(cfg->nphoton==0) cfg->nphoton=count;
     comm=fgets(comment,MAX_PATH_LENGTH,in);
     if(in==stdin)
     	fprintf(stdout,"%ld\nPlease specify the random number generator seed: [1234567]\n\t",cfg->nphoton);
     if(cfg->seed==0){
        imesh_ASSERT(fscanf(in,"%d", &(cfg->seed) )==1);
     }else{
        imesh_ASSERT(fscanf(in,"%d", &itmp )==1);
     }
     comm=fgets(comment,MAX_PATH_LENGTH,in);
     if(in==stdin)
     	fprintf(stdout,"%d\nPlease specify the position of the source (in grid unit): [10 10 5]\n\t",cfg->seed);
     imesh_ASSERT(fscanf(in,"%f %f %f", &(cfg->srcpos.x),&(cfg->srcpos.y),&(cfg->srcpos.z) )==3);
     comm=fgets(comment,MAX_PATH_LENGTH,in);
     ...
     imesh_ASSERT(fscanf(in,"%f %f %f", &(cfg->srcdir.x),&(cfg->srcdir.y),&(cfg->srcdir.z) )==3);
     comm=fgets(comment,MAX_PATH_LENGTH,in);
     ...
     imesh_ASSERT(fscanf(in,"%f %f %f", &(cfg->tstart),&(cfg->tend),&(cfg->tstep) )==3);
     comm=fgets(comment,MAX_PATH_LENGTH,in);
     ...
     if(cfg->tstart>cfg->tend || cfg->tstep==0.f){
         imesh_ERROR(-9,"incorrect time gate settings");
     }
     ...
     imesh_ASSERT(fscanf(in,"%s", filename)==1);
     if(cfg->rootpath[0]){
#ifdef WIN32
     ...
     imesh_ASSERT(fscanf(in,"%f %d %d %d", &(cfg->steps.x),&(cfg->dim.x),&(cfg->crop0.x),&(cfg->crop1.x))==4);
     comm=fgets(comment,MAX_PATH_LENGTH,in);
     ...
     imesh_ASSERT(fscanf(in,"%f %d %d %d", &(cfg->steps.y),&(cfg->dim.y),&(cfg->crop0.y),&(cfg->crop1.y))==4);
     comm=fgets(comment,MAX_PATH_LENGTH,in);
     ...
     imesh_ASSERT(fscanf(in,"%f %d %d %d", &(cfg->steps.z),&(cfg->dim.z),&(cfg->crop0.z),&(cfg->crop1.z))==4);
     comm=fgets(comment,MAX_PATH_LENGTH,in);

     if(cfg->steps.x!=cfg->steps.y || cfg->steps.y!=cfg->steps.z)
        imesh_ERROR(-9,"MCX currently does not support anisotropic voxels");
     ...
     imesh_ASSERT(fscanf(in,"%d", &(cfg->medianum))==1);
     cfg->medianum++;
     comm=fgets(comment,MAX_PATH_LENGTH,in);
     ...
     for(i=1;i<cfg->medianum;i++){
		...
		fprintf(stdout,"Please define medium #%d: mus(1/mm), anisotropy, mua(1/mm) and refractive index: [1.01 0.01 0.04 1.37]\n\t",i);
     	imesh_ASSERT(fscanf(in, "%f %f %f %f", &(cfg->prop[i].mus),&(cfg->prop[i].g),&(cfg->prop[i].mua),&(cfg->prop[i].n))==4);
        comm=fgets(comment,MAX_PATH_LENGTH,in);
        if(in==stdin)
     ...
     imesh_ASSERT(fscanf(in,"%d %f", &(cfg->detnum), &(cfg->detradius))==2);
     comm=fgets(comment,MAX_PATH_LENGTH,in);
     if(in==stdin)
     	fprintf(stdout,"%d %f\n",cfg->detnum,cfg->detradius);
     if(cfg->medianum+cfg->detnum>MAX_PROP_AND_DETECTORS)
         imesh_ERROR(-4,"input media types plus detector number exceeds the maximum total (4000)");
     ...
     for(i=0;i<cfg->detnum;i++){
		...
		fprintf(stdout,"Please define detector #%d: x,y,z (in grid unit): [5 5 5 1]\n\t",i);
     	imesh_ASSERT(fscanf(in, "%f %f %f", &(cfg->detpos[i].x),&(cfg->detpos[i].y),&(cfg->detpos[i].z))==3);
	cfg->detpos[i].w=cfg->detradius;
     ...
     if(fscanf(in,"%s", strtypestr)==1 && strtypestr[0]){
        int srctype=imesh_keylookup(strtypestr,srctypeid);
	if(srctype==-1)
	   imesh_ERROR(-6,"the specified source type is not supported");
        if(srctype>=0){
           comm=fgets(comment,MAX_PATH_LENGTH,in);
	   cfg->srctype=srctype;
	   if(in==stdin)
     	      fprintf(stdout,"Please specify the source parameters set 1 (4 floating-points):\n\t");
           imesh_assert(fscanf(in, "%f %f %f %f", &(cfg->srcparam1.x),
	   ...
	   if(in==stdin)
           ...
           imesh_assert(fscanf(in, "%f %f %f %f", &(cfg->srcparam2.x),
	          &(cfg->srcparam2.y),&(cfg->srcparam2.z),&(cfg->srcparam2.w))==4);
           if(cfg->srctype==imesh_SRC_PATTERN && cfg->srcparam1.w*cfg->srcparam2.w>0){
               ...
               imesh_ASSERT(fscanf(in, "%s", patternfile)==1);
               fp=fopen(patternfile,"rb");
               if(fp==NULL)
                     imesh_ERROR(-6,"pattern file can not be opened");
               imesh_ASSERT(fread(cfg->srcpattern,cfg->srcparam1.w*cfg->srcparam2.w*cfg->srcnum,sizeof(float),fp)==sizeof(float));
               fclose(fp);
           }else if(cfg->srctype==imesh_SRC_PATTERN3D && cfg->srcparam1.x*cfg->srcparam1.y*cfg->srcparam1.z>0){
               char patternfile[MAX_PATH_LENGTH];
               FILE *fp;
               if(cfg->srcpattern) free(cfg->srcpattern);
               ...
               if(fp==NULL)
                     imesh_ERROR(-6,"pattern file can not be opened");
               imesh_ASSERT(fread(cfg->srcpattern,cfg->srcparam1.x*cfg->srcparam1.y*cfg->srcparam1.z*cfg->srcnum,sizeof(float),fp)==sizeof(float));
               fclose(fp);
           }
	}else
	   return;
...
int imesh_loadjson(cJSON *root, Config *cfg){
     int i;
	...
	if(val){
            cfg->mediabyte=imesh_keylookup((char*)FIND_JSON_KEY("MediaFormat","Domain.MediaFormat",Domain,"byte",valuestring),mediaformat);
	    if(cfg->mediabyte==-1)
	       imesh_ERROR(-1,"Unsupported media format.");
        ...
        if(meds){
           ...
           if(med){
             ...
             for(i=0;i<cfg->medianum;i++){
               cJSON *val=FIND_JSON_OBJ("mua",(imesh_ERROR(-1,"You must specify absorption coeff, default in 1/mm"),""),med);
               if(val) cfg->prop[i].mua=val->valuedouble;
	       val=FIND_JSON_OBJ("mus",(imesh_ERROR(-1,"You must specify scattering coeff, default in 1/mm"),""),med);
               if(val) cfg->prop[i].mus=val->valuedouble;
	       val=FIND_JSON_OBJ("g",(imesh_ERROR(-1,"You must specify anisotropy [0-1]"),""),med);
               if(val) cfg->prop[i].g=val->valuedouble;
	       val=FIND_JSON_OBJ("n",(imesh_ERROR(-1,"You must specify refractive index"),""),med);
	...
	if(val && cJSON_GetArraySize(val)>=3){
	   ...
	   if(!Shapes)
	      imesh_ERROR(-1,"You must specify the dimension of the volume");
	}
	...
	if(val){
	   if(cJSON_GetArraySize(val)>=3){
	       ...
	       imesh_ERROR(-1,"Domain::Step has incorrect element numbers");
           }
	}
	if(cfg->steps.x!=cfg->steps.y || cfg->steps.y!=cfg->steps.z)
           imesh_ERROR(-9,"MCX currently does not support anisotropic voxels");
	...
	if(val){
	   ...
	   if(cJSON_GetArraySize(val)>=1){
	       ...
	       if(len==1)
	           ...
	           imesh_ERROR(-1,"Domain::VoxelSize::Dx has incorrect element numbers");
	   ...
	   if(cJSON_GetArraySize(val)>=1){
	       ...
	       if(len==1)
	           ...
	           imesh_ERROR(-1,"Domain::VoxelSize::Dy has incorrect element numbers");
	   ...
	   if(cJSON_GetArraySize(val)>=1){
	       ...
	       if(len==1)
	           ...
	           imesh_ERROR(-1,"Domain::VoxelSize::Dz has incorrect element numbers");
	...
	if(val){
	   if(cJSON_GetArraySize(val)>=3){
	       ...
	       imesh_ERROR(-1,"Domain::CacheBoxP0 has incorrect element numbers");
           }
	}
	...
	if(val){
	   if(cJSON_GetArraySize(val)>=3){
	       ...
	       imesh_ERROR(-1,"Domain::CacheBoxP1 has incorrect element numbers");
           }
	}
	...
	if(cfg->sradius>0.f){
     ...
     if(Optode){
	   ...
	   if(!cfg->issrcfrom0){
           ...
           cfg->srctype=imesh_keylookup((char*)FIND_JSON_KEY("Type","Optode.Source.Type",src,"pencil",valuestring),srctypeid);
              ...
              if(nx>0 && ny>0){
                 ...
                 if(pat && pat->child){
                     ...
                     for(i=0;i<nx*ny*nz*cfg->srcnum;i++){
                         if(pat==NULL)
                             imesh_ERROR(-1,"Incomplete pattern data");
	...
	if(!flagset['D']){
	    if(FIND_JSON_KEY("DebugFlag","Session.DebugFlag",Session,"",valuestring))
	       cfg->debuglevel=imesh_parsedebugopt(FIND_JSON_KEY("DebugFlag","Session.DebugFlag",Session,"",valuestring),debugflag);
	    else
	...
	if(!flagset['w']){
	    if(FIND_JSON_KEY("SaveDataMask","Session.SaveDataMask",Session,"DP",valuestring))
	        cfg->savedetflag=imesh_parsedebugopt(FIND_JSON_KEY("SaveDataMask","Session.SaveDataMask",Session,"DP",valuestring),saveflag);
	    else
	        cfg->savedetflag=FIND_JSON_KEY("SaveDataMask","Session.SaveDataMask",Session,5,valueint);
	}

        if(!cfg->outputformat)  cfg->outputformat=imesh_keylookup((char *)FIND_JSON_KEY("OutputFormat","Session.OutputFormat",Session,"mc2",valuestring),outputformat);
        if(cfg->outputformat<0)
                imesh_ERROR(-2,"the specified output format is not recognized");

        strncpy(val,FIND_JSON_KEY("OutputType","Session.OutputType",Session,outputtype+cfg->outputtype,valuestring),1);
        if(imesh_lookupindex(val, outputtype)){
                imesh_ERROR(-2,"the specified output data type is not recognized");
        }
	...
	if(cfg->tstart>cfg->tend || cfg->tstep==0.f)
     ...
     if(filename[0]=='\0'){
		 ...
		 char *type=NULL, *buf=NULL;
	         if(imesh_jdatadecode((void **)&buf, &ndim, &(cfg->dim.x), 3, &type, Shapes,cfg)==0)
		     imesh_loadvolume(buf,cfg,1);
		 if(buf)
		     free(buf);
	     }else{
		 int status;
		 Grid3D grid={&(cfg->vol),&(cfg->dim),{1.f,1.f,1.f},cfg->isrowmajor};
		 if(cfg->issrcfrom0) memset(&(grid.orig.x),0,sizeof(float3));
		 status=imesh_parse_jsonshapes(root, &grid);
		 if(status){
		     imesh_ERROR(status,imesh_last_shapeerror());
		 }
	     }
	 }else{
	     imesh_ERROR(-1,"You must either define Domain.VolumeFile, or define a Shapes section");
	 }
     }else if(Shapes){
         imesh_ERROR(-1,"You can not specify both Domain.VolumeFile and Shapes sections");
     }
     imesh_prepdomain(filename,cfg);
     cfg->his.maxmedia=cfg->medianum-1; /*skip media 0*/
     cfg->his.detnum=cfg->detnum;
...
void imesh_saveconfig(FILE *out, Config *cfg){
     uint i;
...
void imesh_savejdata(char *filename, Config *cfg){
	...
	cJSON_AddStringToObject(obj, "OutputType", outputtypestr);
	 ...
	 if(shape==NULL)
		imesh_ERROR(-1,"the input shape construct is not a valid JSON object");
	 ...
	 imesh_convertcol2row((uint **)(&buf),&cfg->dim);
         vol=(uint *)buf;
	 ...
	 ret=imesh_jdataencode(buf,3,&cfg->dim.x,(cfg->mediabyte==1 ? "uint8" : (cfg->mediabyte==2 ? "uint16" : "uint32")),
	     outputbytes/datalen, cfg->zipid, obj, 0, cfg);
         if(buf)
             free(buf);

	 if(ret)
	     imesh_ERROR(ret,"data compression or base64 encoding failed");
	 else
...
void imesh_loadvolume(char *filename,Config *cfg,int isbuf){
	 ...
	 if(strstr(filename,".json")!=NULL){
	     ...
	     Grid3D grid={&(cfg->vol),&(cfg->dim),{1.f,1.f,1.f},cfg->isrowmajor};
	     if(cfg->issrcfrom0) memset(&(grid.orig.x),0,sizeof(float3));
	     status=imesh_load_jsonshapes(&grid,filename);
	     if(status){
		 imesh_ERROR(status,imesh_last_shapeerror());
	     }
	     return;
	 ...
	 if(fp==NULL){
		 imesh_ERROR(-5,"the specified binary volume file does not exist");
	 }
     }
     if(cfg->vol){
     	     free(cfg->vol);
	 ...
	 fclose(fp);
	 if(res!=datalen){
	     imesh_ERROR(-6,"file size does not match specified dimensions");
	 }
     }else
     ...
     if(cfg->mediabyte<=4)
       for(i=0;i<datalen;i++){
         if(cfg->vol[i]>=cfg->medianum)
            imesh_ERROR(-6,"medium index exceeds the specified medium types");
     }
...
void imesh_loadseedfile(Config *cfg){
    History his;
    FILE *fp=fopen(cfg->seedfile,"rb");
    if(fp==NULL)
        imesh_ERROR(-7,"can not open the specified history file");
    if(fread(&his,sizeof(History),1,fp)!=1)
        imesh_ERROR(-7,"error when reading the history file");
    if(his.savedphoton==0 || his.seedbyte==0){
	imesh_ERROR(-7,"history file does not contain seed data, please re-run your simulation with '-q 1'");
    }
    if(his.maxmedia!=cfg->medianum-1)
        imesh_ERROR(-7,"the history file was generated with a different media setting");
    if(fseek(fp,his.savedphoton*his.colcount*sizeof(float),SEEK_CUR))
        imesh_ERROR(-7,"illegal history file");
    cfg->replay.seed=malloc(his.savedphoton*his.seedbyte);
    if(cfg->replay.seed==NULL)
        imesh_ERROR(-7,"can not allocate memory");
    if(fread(cfg->replay.seed,his.seedbyte,his.savedphoton,fp)!=his.savedphoton)
        imesh_ERROR(-7,"error when reading the seed data");
    ...
    if(cfg->outputtype==otJacobian || cfg->outputtype==otWP || cfg->outputtype==otDCS ){ //cfg->replaydet>0
       ...
       if(((!hasdetid) && cfg->detnum>1) || !SAVE_PPATH(his.savedetflag))
           imesh_ERROR(-7,"please rerun the baseline simulation and save detector ID (D) and partial-path (P) using '-w DP'");
...
void  imesh_convertrow2col(unsigned int **vol, uint3 *dim){
     uint x,y,z;
...
void  imesh_convertcol2row(unsigned int **vol, uint3 *dim){
     uint x,y,z;
     unsigned int dimxy,dimyz;
     unsigned int *newvol=NULL;
...
void  imesh_convertcol2row4d(unsigned int **vol, uint4 *dim){
     uint x,y,z,w;
     ...
     */
     for(d=0;d<cfg->detnum;d++){                             /*loop over each detector*/
        ...
        if(cfg->issavedet && count==0)
              imesh_FPRINTF(stderr,S_RED "WARNING: detector %d is not located on an interface, please check coordinates.\n" S_RESET,d+1);
     }
...
void imesh_dumpmask(Config *cfg){
     char fname[MAX_FULL_PATH];
     if(cfg->rootpath[0])
	 ...
	 imesh_convertcol2row((uint **)(&buf), (uint3 *)dims);
	 if(cfg->outputformat==ofJNifti)
             imesh_savejnii((float *)buf, 3, dims, voxelsize, fname, 0, cfg);
	 else
	     imesh_savebnii((float *)buf, 3, dims, voxelsize, fname, 0, cfg);
	 free(buf);
     }else
         imesh_savenii((float *)cfg->vol, cfg->dim.x*cfg->dim.y*cfg->dim.z, fname, NIFTI_TYPE_UINT32, ofNifti, cfg);
     if(cfg->isdumpmask==1 && cfg->isdumpjson==0){ /*if dumpmask>1, simulation will also run*/
         imesh_FPRINTF(cfg->flog,"volume mask is saved in %s\n",fname);
         exit(0);
     }
}
...
int  imesh_jdatadecode(void **vol, int *ndim, uint *dims, int maxdim, char **type, cJSON *obj, Config *cfg){
     int ret=0;
     cJSON * ztype=NULL;
	 ...
	 if(strstr(*type,"int8"))
	     ...
	     imesh_ERROR(-1,"8-byte volume array is not supported");
     }
     if(vdata){
         if(vsize){
	     ...
	     int status=0;
	     char *buf=NULL;
	     int zipid=imesh_keylookup((char *)(ztype->valuestring),zipformat);
	     ...
	     imesh_ERROR(-1,"Only compressed JData array constructs are supported");
     }else
         imesh_ERROR(-1,"No _ArrayZipData_ field is found");
     return ret;
}
...
int  imesh_jdataencode(void *vol, int ndim, uint *dims, char *type, int byte, int zipid, void *obj, int isubj, Config *cfg){
     uint datalen=1;
     ...
     if(!cfg->isdumpjson)
        imesh_FPRINTF(stdout,"compressing data [%s] ...", zipformat[zipid]);

     /*compress data using zlib*/
     ret=zmat_encode(totalbytes, (uchar *)vol, &compressedbytes, (uchar **)&compressed, zipid, &status);
     if(!ret){
         if(!cfg->isdumpjson)
             imesh_FPRINTF(stdout,"compression ratio: %.1f%%\t",compressedbytes*100.f/totalbytes);
	 if(isubj){
	     ...
	     if(!cfg->isdumpjson)
		 imesh_FPRINTF(stdout,"after encoding: %.1f%%\n",totalbytes*100.f/(datalen*byte));
	     if(!ret){
...
void imesh_progressbar(float percent, Config *cfg){
    ...
    if(percentage != oldmarker){
        if(percent!=-0.f)
	    for(j=0;j<colwidth;j++)     imesh_FPRINTF(stdout,"\b");
        oldmarker=percentage;
        imesh_FPRINTF(stdout,S_YELLOW"Progress: [");
        for(j=0;j<percentage;j++)      imesh_FPRINTF(stdout,"=");
        imesh_FPRINTF(stdout,(percentage<colwidth-18) ? ">" : "=");
        for(j=percentage;j<colwidth-18;j++) imesh_FPRINTF(stdout," ");
        imesh_FPRINTF(stdout,"] %3d%%" S_RESET,(int)(percent*100));
#ifdef imesh_CONTAINER
        imesh_matlab_flush();
#else
        fflush(stdout);
...
int imesh_readarg(int argc, char *argv[], int id, void *output,const char *type){
     /*
	...
	return id;
     ...
     if(id<argc-1){
	     ...
	     strcpy((char *)output,argv[id+1]);
     	 ...
     	 imesh_ERROR(-1,"incomplete input");
     }
     return id+1;
}
...
void imesh_parsecmd(int argc, char* argv[], Config *cfg){
     ...
     if(argc<=1){
     	imesh_usage(cfg,argv[0]);
     	exit(0);
     }
     while(i<argc){
     	    if(argv[i][0]=='-'){
		if(argv[i][1]=='-'){
			if(imesh_remap(argv[i])){
				imesh_ERROR(-2,"unknown verbose option");
			}
		}
		if(argv[i][1]<='z' && argv[i][1]>='A')
	        ...
	        switch(argv[i][1]){
		     ...
		     case 'i':
				if(filename[0]){
					imesh_ERROR(-2,"you can not specify both interactive mode and config file");
				}
		     		isinteractive=1;
				break;
		     case 'f': 
				...
				if(i<argc-1 && argv[i+1][0]=='{'){
					...
					i++;
		     	        	...
		     	        	i=imesh_readarg(argc,argv,i,filename,"string");
				break;
		     case 'n':
		     	        i=imesh_readarg(argc,argv,i,&(np),"float");
				cfg->nphoton=(size_t)np;
		     	        break;
		     case 't':
		     	        i=imesh_readarg(argc,argv,i,&(cfg->nthread),"int");
		     	        break;
                     case 'T':
                               	i=imesh_readarg(argc,argv,i,&(cfg->nblocksize),"int");
                               	break;
		     case 's':
		     	        i=imesh_readarg(argc,argv,i,cfg->session,"string");
		     	        break;
		     case 'a':
		     	        i=imesh_readarg(argc,argv,i,&(cfg->isrowmajor),"char");
		     	        break;
		     case 'q':
		     	        i=imesh_readarg(argc,argv,i,&(cfg->issaveseed),"char");
		     	        break;
		     case 'g':
		     	        i=imesh_readarg(argc,argv,i,&(cfg->maxgate),"int");
		     	        break;
		     case 'b':
		     	        i=imesh_readarg(argc,argv,i,&(cfg->isreflect),"char");
				...
				i++;
                               	break;
		     case 'd':
		     	        i=imesh_readarg(argc,argv,i,&(cfg->issavedet),"char");
		     	        break;
		     case 'm':
		                i=imesh_readarg(argc,argv,i,&(cfg->ismomentum),"char");
				if (cfg->ismomentum) cfg->issavedet=1;
				break;
		     case 'r':
		     	        i=imesh_readarg(argc,argv,i,&(cfg->respin),"int");
		     	        break;
		     case 'S':
		     	        i=imesh_readarg(argc,argv,i,&(cfg->issave2pt),"char");
		     	        break;
		     case 'p':
		     	        i=imesh_readarg(argc,argv,i,&(cfg->printnum),"int");
		     	        break;
                     case 'e':
		     	        i=imesh_readarg(argc,argv,i,&(cfg->minenergy),"float");
                                break;
		     case 'U':
		     	        i=imesh_readarg(argc,argv,i,&(cfg->isnormalized),"char");
		     	        break;
                     case 'R':
                                i=imesh_readarg(argc,argv,i,&(cfg->sradius),"float");
                                break;
                     case 'u':
                                i=imesh_readarg(argc,argv,i,&(cfg->unitinmm),"float");
                                break;
                     case 'l':
                                issavelog=1;
                                break;
		     ...
		     case 'I':
		                ...
		                break;
                     case 'G':
                                if(imesh_isbinstr(argv[i+1])){
                                    i=imesh_readarg(argc,argv,i,cfg->deviceid,"string");
                                    break;
                                }else{
                                    i=imesh_readarg(argc,argv,i,&(cfg->gpuid),"int");
                                    memset(cfg->deviceid,'0',MAX_DEVICE);
                                    if(cfg->gpuid>0 && cfg->gpuid<MAX_DEVICE)
                                         cfg->deviceid[cfg->gpuid-1]='1';
                                    else
                                         imesh_ERROR(-2,"GPU id can not be more than 256");
                                    break;
                                }
                     case 'W':
                                i=imesh_readarg(argc,argv,i,cfg->workload,"floatlist");
                                break;
                     case 'z':
                                i=imesh_readarg(argc,argv,i,&(cfg->issrcfrom0),"char");
                                break;
		     case 'M':
		     	        i=imesh_readarg(argc,argv,i,&(cfg->isdumpmask),"char");
		     	        break;
                     case 'Y':
                                i=imesh_readarg(argc,argv,i,&(cfg->replaydet),"int");
                                break;
		     case 'H':
		     	        i=imesh_readarg(argc,argv,i,&(cfg->maxdetphoton),"int");
		     	        break;
                     case 'P':
		                len=strlen(argv[i]);
                                if(cfg->shapedata)
				...
				memcpy(cfg->shapedata,argv[++i],len);
                                break;
                     case 'A':
                                i=imesh_readarg(argc,argv,i,&(cfg->autopilot),"char");
                                break;
                     case 'E':
				if(i<argc-1 && strstr(argv[i+1],".mch")!=NULL){ /*give an mch file to initialize the seed*/
#if defined(USE_LL5_RAND)
					imesh_ERROR(-1,"seeding file is not supported in this binary");
#else
                                        i=imesh_readarg(argc,argv,i,cfg->seedfile,"string");
					cfg->seed=SEED_FROM_FILE;
#endif
		     	        }else
					i=imesh_readarg(argc,argv,i,&(cfg->seed),"int");
		     	        break;
                     case 'O':
                                i=imesh_readarg(argc,argv,i,&(cfg->outputtype),"string");
				if(imesh_lookupindex(&(cfg->outputtype), outputtype)){
                                        imesh_ERROR(-2,"the specified output data type is not recognized");
                                }
                                break;
                     case 'k':
                                i=imesh_readarg(argc,argv,i,&(cfg->voidtime),"int");
                                break;
		     case 'V':
		     	        i=imesh_readarg(argc,argv,i,&(cfg->isspecular),"char");
		     	        break;
                     case 'v':
                                imesh_version(cfg);
				break;
                     case 'D':
                                if(i+1<argc && isalpha(argv[i+1][0]) )
                                        cfg->debuglevel=imesh_parsedebugopt(argv[++i],debugflag);
                                else
                                        i=imesh_readarg(argc,argv,i,&(cfg->debuglevel),"int");
                                break;
		     case 'F':
                                if(i>=argc)
                                        imesh_ERROR(-1,"incomplete input");
                                if((cfg->outputformat=imesh_keylookup(argv[++i], outputformat))<0)
                                        imesh_ERROR(-2,"the specified output data type is not recognized");
                                break;
		     case 'x':
 		                i=imesh_readarg(argc,argv,i,&(cfg->issaveexit),"char");
 				if (cfg->issaveexit) cfg->issavedet=1;
 				break;
		     case 'X':
 		                i=imesh_readarg(argc,argv,i,&(cfg->issaveref),"char");
 				if (cfg->issaveref) cfg->issaveref=1;
 				break;
		     case 'Z':
			        if(i+1<argc && isalpha(argv[i+1][0]) ){
				    cfg->zipid=imesh_keylookup(argv[++i],zipformat);
			        }else
				    i=imesh_readarg(argc,argv,i,&(cfg->zipid),"int");
 				break;
		     case 'w':
			        if(i+1<argc && isalpha(argv[i+1][0]) ){
				    cfg->savedetflag=imesh_parsedebugopt(argv[++i],saveflag);
			        }else
				    i=imesh_readarg(argc,argv,i,&(cfg->savedetflag),"int");
 				break;
		     case '-':  /*additional verbose parameters*/
                                if(strcmp(argv[i]+2,"maxvoidstep")==0)
                                     i=imesh_readarg(argc,argv,i,&(cfg->maxvoidstep),"int");
                                else if(strcmp(argv[i]+2,"maxjumpdebug")==0)
                                     i=imesh_readarg(argc,argv,i,&(cfg->maxjumpdebug),"int");
                                else if(strcmp(argv[i]+2,"gscatter")==0)
                                     i=imesh_readarg(argc,argv,i,&(cfg->gscatter),"int");
                                else if(strcmp(argv[i]+2,"mediabyte")==0){
				     if(i+1<argc && isalpha(argv[i+1][0]) ){
				         cfg->mediabyte=imesh_keylookup(argv[++i],mediaformat);
					 if(cfg->mediabyte==-1)
					     imesh_ERROR(-1,"Unsupported media format.");
					 cfg->mediabyte=mediaformatid[cfg->mediabyte];
			             }else
				         i=imesh_readarg(argc,argv,i,&(cfg->mediabyte),"int");
                                }else if(strcmp(argv[i]+2,"faststep")==0)
                                     i=imesh_readarg(argc,argv,i,&(cfg->faststep),"char");
                                else if(strcmp(argv[i]+2,"root")==0)
                                     i=imesh_readarg(argc,argv,i,cfg->rootpath,"string");
					 ...
					 i++;
				         ...
				         i=imesh_readarg(argc,argv,i,&(cfg->isdumpjson),"int");
                                }else if(strcmp(argv[i]+2,"bench")==0){
                                     if(i+1<argc && isalpha(argv[i+1][0]) ){
				         int idx=imesh_keylookup(argv[++i],benchname);
					 if(idx==-1)
					     imesh_ERROR(-1,"Unsupported bechmark.");
					 isinteractive=0;
					 jsoninput=(char *)benchjson[idx];
			             }else{
				         imesh_FPRINTF(cfg->flog,"Built-in benchmarks:\n");
				         for(int i=0;i<sizeof(benchname)/sizeof(char*)-1;i++)
					     imesh_FPRINTF(cfg->flog,"\t%s\n",benchname[i]);
				         exit(0);
				     }
                                }else if(strcmp(argv[i]+2,"reflectin")==0)
                                     i=imesh_readarg(argc,argv,i,&(cfg->isrefint),"char");
                                else if(strcmp(argv[i]+2,"internalsrc")==0)
		                     i=imesh_readarg(argc,argv,i,&(cfg->internalsrc),"int");
                                else
                                     imesh_FPRINTF(cfg->flog,"unknown verbose option: --%s\n",argv[i]+2);
		     	        break;
		}
	    }
	    i++;
     ...
     if(issavelog && cfg->session[0]){
          ...
          if(cfg->flog==NULL){
		cfg->flog=stdout;
		imesh_FPRINTF(cfg->flog,"unable to save to log file, will print from stdout\n");
          }
     }
     if((cfg->outputtype==otJacobian ||cfg->outputtype==otWP || cfg->outputtype==otDCS) && cfg->seed!=SEED_FROM_FILE)
         imesh_ERROR(-1,"Jacobian output is only valid in the reply mode. Please give an mch file after '-E'.");

     if(cfg->isgpuinfo!=2){ /*print gpu info only*/
	  if(isinteractive){
             imesh_readconfig((char*)"",cfg);
	  }else if(jsoninput){
     	     imesh_readconfig(jsoninput,cfg);
	  }else{
             imesh_readconfig(filename,cfg);
          }
     }
}
...
int imesh_parsedebugopt(char *debugopt,const char *debugflag){
    char *c=debugopt,*p;
    int debuglevel=0;
...
int imesh_keylookup(char *origkey, const char *table[]){
    int i=0;
...
int imesh_lookupindex(char *key, const char *index){
    int i=0;
    while(index[i]!='\0'){
...
void imesh_version(Config *cfg){
    const char ver[]="$Rev::e7ce8f$2019.4";
    int v=0;
    sscanf(ver,"$Rev::%x",&v);
    imesh_FPRINTF(cfg->flog, "MCX Revision %x\n",v);
    exit(0);
}
...
int imesh_isbinstr(const char * str){
...
int imesh_run_from_json(char *jsonstr){
     ...
     imesh_initcfg(&mcxconfig);
     imesh_readconfig(jsonstr, &mcxconfig);

     if(!(activedev=imesh_list_gpu(&mcxconfig,&gpuinfo))){
         imesh_ERROR(-1,"No GPU device found\n");
     }
     ...
     {
#endif
         imesh_run_simulation(&mcxconfig,gpuinfo); 
#ifdef _OPENMP
     }
#endif

     imesh_cleargpuinfo(&gpuinfo);
     imesh_clearcfg(&mcxconfig);
     return 0;
}
...
void imesh_printheader(Config *cfg){
    imesh_FPRINTF(cfg->flog,S_GREEN"\
###############################################################################\n\
...
###############################################################################\n\
$Rev::e7ce8f$2020.4 $Date::2020-05-02 18:18:31 -04$ by $Author::Qianqian Fang $\n\
###############################################################################\n" S_RESET);
}
...
void imesh_usage(Config *cfg,char *exename){
     imesh_printheader(cfg);
     printf("\n\
